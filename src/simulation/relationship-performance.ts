...
type RelationshipProfile = Omit<RelationshipPerformanceMetadata, 'relationshipIntensity' | 'relationshipPartnerId' | 'leadershipStance'>;
...
const profiles: Record<RelationshipPerformanceId, RelationshipProfile> = {
  neutral: {
    relationshipPerformance: 'neutral', relationshipApproachRate: 1, relationshipAnimationRate: 1,
    relationshipLean: 0, relationshipTurn: 0, relationshipDistanceOffset: 0,
    relationshipGaze: 'direct', relationshipGesture: 'neutral', relationshipSpeechStyle: 'neutral',
    relationshipSymbol: '', relationshipColor: '#cbd5e1',
  },
  bonded: {
    relationshipPerformance: 'bonded', relationshipApproachRate: 1.04, relationshipAnimationRate: 0.92,
    relationshipLean: -0.025, relationshipTurn: -0.035, relationshipDistanceOffset: -1.15,
    relationshipGaze: 'soft', relationshipGesture: 'mirror', relationshipSpeechStyle: 'warm',
    relationshipSymbol: '∞', relationshipColor: '#f9a8d4',
  },
  trusting: {
    relationshipPerformance: 'trusting', relationshipApproachRate: 1.02, relationshipAnimationRate: 0.96,
    relationshipLean: -0.015, relationshipTurn: -0.02, relationshipDistanceOffset: -0.55,
    relationshipGaze: 'direct', relationshipGesture: 'open', relationshipSpeechStyle: 'open',
    relationshipSymbol: '○', relationshipColor: '#86efac',
  },
  respectful: {
    relationshipPerformance: 'respectful', relationshipApproachRate: 0.98, relationshipAnimationRate: 0.94,
    relationshipLean: 0.015, relationshipTurn: 0.02, relationshipDistanceOffset: 0.18,
    relationshipGaze: 'attentive', relationshipGesture: 'nod', relationshipSpeechStyle: 'formal',
    relationshipSymbol: '◇', relationshipColor: '#fde68a',
  },
  protective: {
    relationshipPerformance: 'protective', relationshipApproachRate: 1.08, relationshipAnimationRate: 1.03,
    relationshipLean: -0.055, relationshipTurn: -0.045, relationshipDistanceOffset: -0.7,
    relationshipGaze: 'watchful', relationshipGesture: 'shield', relationshipSpeechStyle: 'protective',
    relationshipSymbol: '⛨', relationshipColor: '#67e8f9',
  },
  deferential: {
    relationshipPerformance: 'deferential', relationshipApproachRate: 0.92, relationshipAnimationRate: 0.9,
    relationshipLean: 0.055, relationshipTurn: 0.075, relationshipDistanceOffset: 0.45,
    relationshipGaze: 'lowered', relationshipGesture: 'yield', relationshipSpeechStyle: 'subdued',
    relationshipSymbol: '↓', relationshipColor: '#c4b5fd',
  },
  guarded: {
    relationshipPerformance: 'guarded', relationshipApproachRate: 0.9, relationshipAnimationRate: 0.94,
    relationshipLean: 0.05, relationshipTurn: 0.1, relationshipDistanceOffset: 1.15,
    relationshipGaze: 'sidelong', relationshipGesture: 'closed', relationshipSpeechStyle: 'careful',
    relationshipSymbol: '|', relationshipColor: '#93c5fd',
  },
  intimidated: {
    relationshipPerformance: 'intimidated', relationshipApproachRate: 1.08, relationshipAnimationRate: 1.08,
    relationshipLean: 0.085, relationshipTurn: 0.14, relationshipDistanceOffset: 1.55,
    relationshipGaze: 'avoidant', relationshipGesture: 'flinch', relationshipSpeechStyle: 'hesitant',
    relationshipSymbol: '!', relationshipColor: '#a78bfa',
  },
  resentful: {
    relationshipPerformance: 'resentful', relationshipApproachRate: 1.04, relationshipAnimationRate: 1.08,
    relationshipLean: -0.055, relationshipTurn: 0.08, relationshipDistanceOffset: 0.85,
    relationshipGaze: 'hard', relationshipGesture: 'dismiss', relationshipSpeechStyle: 'cold',
    relationshipSymbol: '×', relationshipColor: '#fb7185',
  },
  rivalrous: {
    relationshipPerformance: 'rivalrous', relationshipApproachRate: 1.12, relationshipAnimationRate: 1.1,
    relationshipLean: -0.075, relationshipTurn: -0.08, relationshipDistanceOffset: -0.15,
    relationshipGaze: 'challenging', relationshipGesture: 'challenge', relationshipSpeechStyle: 'competitive',
    relationshipSymbol: '⇄', relationshipColor: '#f59e0b',
  },
};
...
const partnerFor = (world: WorldState, heroId: string, directive?: EmotionallyPerformedDirective): string | undefined => {
  ...
  const leadership = (world as LeadershipWorld).leadership;
  if (directive?.formation === 'line' || directive?.formation === 'table') {
    const leaderId = leadership?.familyLeaderId;
    if (leaderId && leaderId !== heroId) return leaderId;
  }
  return world.heroes[heroId]?.currentAction?.targetId;
};

export const relationshipPerformanceForHero = (
  world: WorldState,
  heroId: string,
  partnerId?: string,
): RelationshipPerformanceMetadata => {
  const hero = world.heroes[heroId];
  const partner = partnerId ? world.heroes[partnerId] : undefined;
  if (!hero || !partner || partner.id === hero.id) {
    return { ...profiles.neutral, relationshipIntensity: 0, relationshipPartnerId: partnerId, leadershipStance: 'none' };
  }
  ...
  const leadership = leadershipContext(world, hero.id, partner.id);
  ...
  const scores: Record<Exclude<RelationshipPerformanceId, 'neutral'>, number> = {
    ...
    respectful: Math.max(0, respect) * 0.86 + Math.max(0, trust) * 0.18 + leadership.authority * 0.22,
    protective: Math.max(0, closeness) * 0.4 + Math.max(0, liking) * 0.32 + Math.max(0, trust) * 0.25
      + hero.traits.empathy * 0.22 + partner.condition.injury * 0.42,
    deferential: leadership.authority * 0.52 + leadership.obedience * 0.32 + leadership.politicalLoyalty * 0.24
      + hero.traits.loyalty * 0.14 - hero.traits.independence * 0.16,
    guarded: Math.max(0, 35 - trust) * 1.08 + hero.traits.caution * 0.28 + Math.max(0, resentment) * 0.34 + Math.max(0, fear) * 0.18,
    intimidated: Math.max(0, fear) * 0.9 + leadership.authority * 0.24 + Math.max(0, 100 - hero.psyche.security) * 0.24
      + hero.emotions.fear * 0.22,
    resentful: Math.max(0, resentment) * 0.94 + hero.emotions.anger * 0.24 + leadership.grievance * 0.4,
    rivalrous: Math.max(0, rivalry) * 0.9 + Math.max(0, envy) * 0.42 + hero.traits.ambition * 0.2 + hero.traits.pride * 0.14,
  };

  if (leadership.stance === 'challenger') scores.rivalrous += 34;
  if (leadership.stance === 'skeptical') { scores.guarded += 18; scores.resentful += 12; }
  if (leadership.stance === 'loyal') { scores.respectful += 18; scores.deferential += 12; }
  if (leadership.stance === 'deferential') scores.deferential += 24;
  if (partner.condition.injury >= 28 && liking + trust + closeness > 45) scores.protective += 28;

  const [winner, rawScore] = (Object.entries(scores) as [Exclude<RelationshipPerformanceId, 'neutral'>, number][])
    .sort((left, right) => right[1] - left[1])[0];
  const id: RelationshipPerformanceId = rawScore >= 42 ? winner : 'neutral';
  const intensity = id === 'neutral' ? clamp(rawScore * 0.42) : clamp(rawScore);
  return {
    ...profiles[id],
    relationshipIntensity: intensity,
    relationshipPartnerId: partner.id,
    leadershipStance: leadership.stance,
  };
};

const relationallyOffsetPosition = (
  directive: EmotionallyPerformedDirective,
  performance: RelationshipPerformanceMetadata,
): { x: number; y: number } => {
...
export const performRelationship = (
  world: WorldState,
  heroId: string,
  directive: EmotionallyPerformedDirective | undefined,
): RelationallyPerformedDirective | undefined => {
