...
const traits = (values: Partial<NumberMap<TraitId>>): NumberMap<TraitId> => ({
  ...
  curiosity: 50, discipline: 50, courage: 50, caution: 50, impulsiveness: 50,
  empathy: 50, independence: 50, approvalSeeking: 50, trustfulness: 50,
  vengefulness: 30, ambition: 50, loyalty: 50, ...values,
});

const emotions = (values: Partial<NumberMap<EmotionId>> = {}): NumberMap<EmotionId> => ({
  joy: 30, sadness: 5, anxiety: 10, anger: 0, irritation: 5, guilt: 0, shame: 0,
  fear: 5, hope: 45, interest: 40, loneliness: 15, inspiration: 25, affection: 20,
  envy: 0, ...values,
});

const needs = (values: Partial<NumberMap<NeedId>> = {}): NumberMap<NeedId> => ({
  hunger: 25, fatigue: 20, safety: 10, social: 25, solitude: 15, recognition: 30,
  growth: 45, belonging: 25, ...values,
});

const psyche = (values: Partial<NumberMap<PsycheId>> = {}): NumberMap<PsycheId> => ({
  stress: 15, confidence: 50, security: 55, grief: 0, burnout: 5, resilience: 55,
  ...values,
});

const stats = (values: Partial<NumberMap<StatId>> = {}): NumberMap<StatId> => ({
  strength: 18, endurance: 18, dexterity: 18, magic: 10, perception: 16, ...values,
});

const relationValues = (
  values: Partial<NumberMap<RelationshipId>> = {},
): NumberMap<RelationshipId> => ({
  liking: 0, trust: 0, respect: 0, closeness: 0, fear: 0, resentment: 0, envy: 0,
  attraction: 0, debt: 0, rivalry: 0, ...values,
});
...
export const createInitialWorld = (seed = 'aster-family-001'): WorldState => {
  ...
  const mira = createHero({
    ...
    inventory: [
      ...
      { id: 'bandage', name: 'Перевязочный набор', quantity: 2, category: 'consumable' },
    ],
    goals: [{ id: 'mira-healer', label: 'Стать человеком, на которого можно положиться', priority: 82, tags: ['care', 'family'] }],
    relationships: {
  ...
  const kael = createHero({
    id: 'kael', name: 'Каэль', age: 20,
    traits: traits({ pride: 87, ambition: 91, discipline: 79, independence: 84, courage: 80, friendliness: 34, patience: 29, empathy: 41, approvalSeeking: 27, vengefulness: 62, cruelty: 38 }),
    ...
    inventory: [
      ...
      { id: 'leather-vest', name: 'Кожаный жилет', quantity: 1, category: 'armor' },
    ],
    goals: [{ id: 'kael-strongest', label: 'Стать сильнейшим ребёнком семьи', priority: 95, tags: ['power', 'status'] }],
    relationships: {
  ...
  const liora = createHero({
    id: 'liora', name: 'Лиора', age: 17,
    traits: traits({ curiosity: 94, friendliness: 69, honesty: 77, kindness: 62, impulsiveness: 71, courage: 44, caution: 37, empathy: 68, trustfulness: 73, approvalSeeking: 74, discipline: 32, pride: 43 }),
    ...
    inventory: [
      ...
      { id: 'primer', name: 'Ветхий учебник магии', quantity: 1, category: 'book' },
    ],
    goals: [{ id: 'liora-magic', label: 'Открыть собственную магию', priority: 90, tags: ['knowledge', 'magic'] }],
    relationships: {
