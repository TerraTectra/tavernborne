...
type Exploration = NonNullable<ReturnType<typeof dungeonExplorationOf>>;
type DungeonRoom = Exploration['rooms'][number];
...
const phaseLabels: Record<DungeonExplorationPhase, string> = {
  entering: 'Вход на этаж', scouting: 'Разведка впереди строя', choosing: 'Выбор маршрута',
  crossing: 'Преодоление прохода', looting: 'Осмотр находки', assessing: 'Оценка угрозы',
  returning: 'Возвращение к выходу', completed: 'Исследование завершено',
};

const roleLabels: Record<DungeonRole, string> = {
  leader: 'лидер', vanguard: 'авангард', scout: 'разведчик', support: 'поддержка',
};

const statusLabels: Record<DungeonActorStatus, string> = {
  moving: 'идёт в строю', scouting: 'разведывает', guarding: 'держит позицию',
  searching: 'осматривает', warning: 'предупреждает', helping: 'помогает', returning: 'возвращается',
};

const roleColor: Record<DungeonRole, string> = {
  leader: '#f2c66d', vanguard: '#ee795d', scout: '#62b8ef', support: '#a88ce8',
};

const roomColor: Record<DungeonRoom['kind'], string> = {
  entrance: '#44525d', hall: '#3d464d', fork: '#46525a', trap: '#604d35',
  cache: '#4d503c', enemy: '#50383b', refuge: '#3c514c',
};
...
function Corridor({ from, to, revealed, route }: { from: DungeonRoom; to: DungeonRoom; revealed: boolean; route: boolean }) {
  ...
  return (
    <group>
      <mesh receiveShadow position={[(fx + tx) / 2, revealed ? 0.01 : -0.02, (fz + tz) / 2]} rotation={[0, angle, 0]}>
...
function RoomFeature({ room, exploration }: { room: DungeonRoom; exploration: Exploration }) {
...
function DungeonRoom3D({ room, exploration }: { room: DungeonRoom; exploration: Exploration }) {
  const [x, , z] = mapPoint(room.x, room.y);
  const width = Math.max(1.5, room.width * 0.17);
  const depth = Math.max(1.35, room.height * 0.13);
  const discovered = Boolean(room.discovered);
  return (
    <group position={[x, 0, z]}>
      <mesh receiveShadow position={[0, -0.02, 0]}><boxGeometry args={[width, 0.18, depth]} /><meshStandardMaterial color={discovered ? roomColor[room.kind] : '#11151a'} roughness={1} /></mesh>
      {discovered ? (
        <>
        ...
        <>
          <mesh position={[0, 0.56, 0]}><boxGeometry args={[width * 0.94, 1.25, depth * 0.94]} /><meshStandardMaterial color="#080b10" transparent opacity={0.86} side={DoubleSide} /></mesh>
          <Sparkles count={28} scale={[width, 1.1, depth]} size={3} speed={0.09} color="#263342" position={[0, 0.65, 0]} />
          <Html center position={[0, 0.9, 0]} zIndexRange={[15, 5]}><div className="dungeon3d-fog-label" data-testid={`dungeon-room-${room.id}`} data-discovered="false" data-visited="false">?</div></Html>
        </>
      )}
    </group>
  );
...
function DungeonScene({ world, expedition }: DungeonWorld3DProps) {
  ...
  const roomsById = Object.fromEntries(exploration.rooms.map((room) => [room.id, room])) as Record<string, DungeonRoom>;
  ...
  return (
    <>
      ...
      {exploration.corridors.map((corridor) => {
        const from = roomsById[corridor.fromId]; const to = roomsById[corridor.toId];
        if (!from || !to) return null;
        const route = routePairs.has(`${corridor.fromId}:${corridor.toId}`) || routePairs.has(`${corridor.toId}:${corridor.fromId}`);
        return <Corridor key={corridor.id} from={from} to={to} revealed={Boolean(from.discovered && to.discovered)} route={route} />;
      })}
      ...
      {Object.values(exploration.actors).map((dungeonActor) => {
        const hero = world.heroes[dungeonActor.heroId]; if (!hero) return null;
        const phase: RuntimeActor['phase'] = dungeonActor.status === 'moving' || dungeonActor.status === 'returning' ? 'moving' : 'acting';
        const actor = { heroId: hero.id, phase, facing: 'down' as const, bubble: dungeonActor.bubble, reaction: dungeonActor.reaction, roleLabel: roleLabels[dungeonActor.role], actionId: 'dungeon' as const };
        const position = mapPoint(dungeonActor.x, dungeonActor.y);
        return (
          <group key={hero.id}>
            <HeroBody3D hero={hero} actor={actor} position={position} compact testIdPrefix="dungeon-hero-3d" />
            <Html center position={[position[0], 0.12, position[2]]} zIndexRange={[2, 1]}><span className="dungeon3d-actor-probe" data-testid={`dungeon-party-${hero.id}`} data-x={dungeonActor.x.toFixed(2)} data-y={dungeonActor.y.toFixed(2)} data-role={dungeonActor.role} data-status={dungeonActor.status} style={{ borderColor: roleColor[dungeonActor.role] }}>{statusLabels[dungeonActor.status]}</span></Html>
          </group>
        );
      })}
...
export function DungeonWorld3D({ world, expedition }: DungeonWorld3DProps) {
  ...
  if (!exploration) return <section className="grid min-h-[720px] place-items-center rounded-3xl border border-sky-200/10 bg-[#090d12] text-sm text-slate-500" data-testid="dungeon-rts-map">Группа проходит через входной шлюз подземелья…</section>;
  ...
  return (
    <section className="world3d-dungeon-shell relative min-h-[720px] overflow-hidden rounded-3xl border border-sky-200/15 bg-[#070b10] shadow-2xl" data-testid="dungeon-rts-map">
      ...
      <aside className="world3d-dungeon-decision" data-testid="dungeon-route-decision"><div><p>Решение группы</p><strong>{latestDecision?.text ?? 'Группа только вошла на этаж и проверяет построение.'}</strong></div><span>шаг {exploration.step}/7</span><footer>{exploration.routeChoice && <i>маршрут: {exploration.routeChoice === 'short-risky' ? 'короткий рискованный' : 'длинный безопасный'}</i>}{exploration.trapDetected !== undefined && <i>ловушка: {exploration.trapDetected ? 'обнаружена' : 'задета'}</i>}{exploration.chestOpened && <i>сундук открыт</i>}{exploration.enemySpotted && <i>враг замечен</i>}{exploration.threatDecision && <i>решение: {exploration.threatDecision === 'avoid' ? 'обойти' : 'отступить'}</i>}</footer></aside>
    </section>
  );
}
