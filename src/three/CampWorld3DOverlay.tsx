...
function WagonHome() {
  return (
    <group position={[0.4, 0, -4.65]} rotation={[0, -0.05, 0]}>
      ...
      <group position={[0.95, 2.9, -0.2]}>
        <mesh castShadow><cylinderGeometry args={[0.14, 0.18, 0.92, 10]} /><meshStandardMaterial color="#2e2925" roughness={0.82} /></mesh>
        <mesh castShadow position={[0, 0.5, 0]}><cylinderGeometry args={[0.22, 0.16, 0.18, 10]} /><meshStandardMaterial color="#3e3932" roughness={0.82} /></mesh>
      </group>
      <group position={[2.1, 1.45, -1.23]}>
        <mesh castShadow position={[0, 0.45, 0]}><boxGeometry args={[0.08, 1.85, 0.08]} /><meshStandardMaterial color="#71482b" roughness={0.86} /></mesh>
        <mesh castShadow position={[0, 0.28, 0.04]}><boxGeometry args={[1.08, 1.4, 0.08]} /><meshStandardMaterial color="#244b5b" roughness={0.72} /></mesh>
        <mesh position={[0, 0.28, -0.02]} rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[0.28, 0.035, 8, 16]} /><meshStandardMaterial color="#e6c66c" emissive="#a37d22" emissiveIntensity={0.2} /></mesh>
      </group>
...
function Workshop() {
  return (
    <group position={[5.3, 0, -0.5]} rotation={[0, -0.15, 0]}>
      ...
      <group position={[1.35, 0, -0.25]}><mesh castShadow position={[0, 0.54, 0]}><cylinderGeometry args={[0.28, 0.32, 1.08, 14]} /><meshStandardMaterial color="#744627" roughness={0.9} /></mesh></group>
    </group>
  );
}
  ...
  return (
    <group position={[-5.35, 0, -0.45]}>
      ...
      <group position={[0.72, 0, -0.25]}>
        {[-0.34, 0, 0.34].map((x, index) => <mesh key={x} castShadow position={[x, 0.82 + index * 0.08, 0]} rotation={[0, 0, index === 1 ? 0.08 : -0.08]}><boxGeometry args={[0.07, 1.65, 0.07]} /><meshStandardMaterial color={index === 1 ? '#a9b5bc' : '#795032'} metalness={index === 1 ? 0.35 : 0} roughness={0.72} /></mesh>)}
      </group>
    </group>
  );
}
...
function QuietCorner() {
  return (
    <group position={[-5.2, 0, 4.35]}>
      <Rock position={[0, 0.3, 0]} scale={1.3} />
      <mesh receiveShadow position={[0.75, 0.05, -0.4]} rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[1.15, 24]} /><meshStandardMaterial color="#35473a" roughness={1} /></mesh>
...
function CampScene({ world, actors, selectedHeroId, onSelectHero }: { world: WorldState; actors: Record<string, RuntimeActor>; selectedHeroId: string; onSelectHero: (id: string) => void }) {
  return (
    <>
      <color attach="background" args={['#101914']} /><fog attach="fog" args={['#101914', 18, 34]} />
      <hemisphereLight args={['#9ac8dd', '#202215', 1.3]} /><ambientLight intensity={0.78} />
...
export function CampWorld3DOverlay() {
  ...
  return createPortal(
    <div className="world3d-layer pointer-events-none absolute inset-0 z-[35] overflow-hidden rounded-3xl" data-testid="camp-3d-layer">
      <Canvas orthographic shadows dpr={[1, 1.5]} camera={{ position: cameraPosition, zoom: 58, near: 0.1, far: 80 }} style={{ pointerEvents: 'none' }} onCreated={({ camera, gl, scene }) => { camera.lookAt(0, 0.45, 0); gl.setClearColor(new Color('#101914')); gl.shadowMap.enabled = true; scene.background = new Color('#101914'); }}>
        <CampScene world={world} actors={actors} selectedHeroId={selectedHeroId} onSelectHero={onSelectHero} />
      </Canvas>
      <div className="world3d-corner-badge" data-testid="camp-3d-status"><span>Живой 3D-лагерь</span><small>ортографическая камера · тела из симуляции</small></div>
    </div>,
    mount,
  );
}
